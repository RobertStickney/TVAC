            ...
            while True:
                ...
                if i > 0:
                    ...
                    if abs(rampRatePerMin) > 12:
                        popupError(
                            "Zone {}, Setpoint {} has ramp rate over 1.2 C per minute. ({}c/min)".format(zone, setPoint,
                                                                                                         rampRatePerMin))
                i += 1
    except (OSError) as e:
        popupError("File named:\n\n{}\n\nCan not be opened, check to make sure file is there and is readable.".format(
            fileName))
        quit()
    ...
    if not demo:
        ...
        if "success" not in r.text:
            ...
            if "1062" in errorCode:
                popupError(
                    "There is already a profile of this name in the database.\nPlease rename the profile or run profile in Database")
            else:
                popupError(r.text)
