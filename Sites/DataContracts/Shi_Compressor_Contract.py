...
class Shi_Compressor_Contract:
    ...
    def getJson(self):
        self.__Lock.acquire()
        message = ['"Helium Return Pressure":%s' % json.dumps(self.return_pressure),
                   '"Helium Discharge Temperature":%s' % json.dumps(self.helium_temp),
                   '"Water Inlet Temperature":%s' % json.dumps(self.water_in_temp),
                   '"Water Outlet Temperature":%s' % json.dumps(self.water_out_temp),
